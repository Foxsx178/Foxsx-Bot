...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "✂️ **Камень, ножницы, бумага**: `/rps [камень/ножницы/бумага]`.\n"
        "🎁 **Бонус**: Жми «Бонус» раз в сутки (+25 монет).\n"
        "🛍 **Магазин**: `/shop` — покупка статусов.\n"
        "🎒 **Экипировка**: `/set [номер]` — надеть купленный статус.\n"
...
async def rps_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("🎮 **Камень, Ножницы, Бумага**\nИспользуй: `/rps камень`, `/rps ножницы` или `/rps бумага`", parse_mode="Markdown")
        return
