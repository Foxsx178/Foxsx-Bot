...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "🎲 **Угадай число**: Пиши /start, жми «Играть» и угадывай число от 1 до 100 (+10 монет).\n"
        "🎰 **Казино**: `/casino [ставка]` — попытай удачу и умножь свои монеты!\n"
        "🎁 **Бонус**: Жми кнопку «Бонус» раз в сутки (+25 монет).\n"
        "🛍 **Магазин**: Покупай крутые звания через `/shop`.\n"
        "🎟 **Промокоды**: Вводи `/promo [код]`, чтобы активировать секретные бонусы."
    )
...
async def casino_play(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("⚠️ Укажи ставку. Пример: `/casino 50`", parse_mode="Markdown")
        return
        
    try:
        bet = int(context.args[0])
    except ValueError:
        await update.message.reply_text("⚠️ Ставка должна быть числом!")
        return
    ...
    if user["balance"] < bet:
        await update.message.reply_text(f"❌ У тебя недостаточно монет! На балансе: {bet} монет.")
        return
        
    # Списываем ставку для начала игры
    user["balance"] -= bet
    
    # Крутим рулетку (эмодзи кубики, слоты или рандомные картинки)
    # Варианты символов для казино
    symbols = ["🍒", "🍋", "🔔", "💎", "7️⃣"]
    ...
    if res1 == res2 == res3:
        if res1 == "7️⃣":
            win = bet * 10  кэшпот х10
            user["balance"] += win
            msg = f"🎰 {slot_result}\n🎉 ДЖЕКПОТ! Твоя ставка умножена на 10!\n💰 Выигрыш: **+{win} монет**"
        else:
            win = bet * 3
            user["balance"] += win
...
# --- Система промокодов ---
# Создать промокод может только админ: /create_promo КОД СУММА
async def create_promo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        return
        
    if len(context.args) < 2:
        await update.message.reply_text("Формат: `/create_promo [код] [сумма]`", parse_mode="Markdown")
        return
...
async def use_promo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if code not in data["promos"]:
        await update.message.reply_text("❌ Такого промокода не существует или он просрочен.")
        return
...
def main():
    ...
    # Админские команды (скрыты из общего меню)
    ...
    app.add_handler(MessageHandler(filters.Regex("^🎰 Казино$"), casino_play)) # Можно просто написать /casino или нажать кнопку
